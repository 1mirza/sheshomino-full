...
import '../technology_content_viewer/technology_quiz_screen.dart';

class TechnologyLessonDetailScreen extends StatefulWidget {
  final int chapterNumber; // This will hold the 'poodeman_number'
  final Lesson lesson;

  const TechnologyLessonDetailScreen({
    super.key,
  ...
  @override
  State<TechnologyLessonDetailScreen> createState() =>
      _TechnologyLessonDetailScreenState();
}

class _TechnologyLessonDetailScreenState
    extends State<TechnologyLessonDetailScreen> {
  ...
  Future<void> _checkContentAvailability() async {
    await Future.wait([
      _checkJsonContent('assets/json_data/json_fanavari/technology_slides.json',
          'slides', (exists) => _hasSlides = exists),
      _checkJsonContent(
          'assets/json_data/json_fanavari/technology_activities.json',
          'activities',
          (exists) => _hasActivities = exists),
      _checkJsonContent('assets/json_data/json_fanavari/technology_quiz.json',
  ...
  Future<void> _checkJsonContent(
    ...
    try {
      ...
      // Use 'poodeman_number' to find the correct section
                    ...
                    onTap: () {
                      ...
                      if (option['page'] == 'slides') {
                        destination = ScienceContentScreen(
                          ...
                          chapterNumber:
                              widget.chapterNumber, // Pass poodeman number
                          lessonNumber: widget.lesson.lessonNumber!,
                          jsonPath:
                              'assets/json_data/json_fanavari/technology_slides.json',
                          contentKey: 'slides',
                          // Tell the screen to use 'poodeman_number' as the key
                          chapterIdentifierKey: 'poodeman_number',
                        ...
                        destination = TechnologyQuizScreen(
                          chapterNumber:
                              widget.chapterNumber, // Pass poodeman number
                          lessonNumber: widget.lesson.lessonNumber!,
