...
class WordFamilyGameScreen extends StatefulWidget {
  final int lessonNumber;
  final String userName;

  const WordFamilyGameScreen({
    super.key,
  ...
  @override
  State<WordFamilyGameScreen> createState() => _WordFamilyGameScreenState();
}

class _WordFamilyGameScreenState extends State<WordFamilyGameScreen> {
  Timer? _timer;
  int _remainingTime = 30;
  List<WordFamily> _allFamilies = [];
  List<WordFamily> _quizFamilies = [];
  int _currentQuestionIndex = 0;
  int _score = 0;
  bool _isLoading = true;

  String _currentQuestionWord = '';
  ...
  Future<void> _loadAndSetupGame() async {
    try {
      ...
      if (lessonData != null) {
        final WordFamilyLesson lesson = WordFamilyLesson.fromJson(lessonData);
        _allFamilies = lesson.families;
        _allFamilies.shuffle();
        _quizFamilies = _allFamilies
            .where((family) => family.words.length > 1)
            .take(10)
            .toList();

        if (_quizFamilies.isNotEmpty) {
          _setupNewQuestion();
        }
      }
    } catch (e) {
  ...
  void _setupNewQuestion() {
    if (_currentQuestionIndex >= _quizFamilies.length) {
      _showResultDialog();
      return;
    }
    ...
    setState(() {
      final currentFamily =
          List<String>.from(_quizFamilies[_currentQuestionIndex].words);
      currentFamily.shuffle();

      _currentQuestionWord = currentFamily[0];
      _currentAnswer = currentFamily[1].replaceAll(' ', '');
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('بازی کلمات هم‌خانواده'),
          leading: IconButton(
  ...
  Widget _buildQuestionCard() {
    return Card(
      child: Padding(
        ...
        child: Text(
          'یک کلمه هم‌خانواده برای "${_currentQuestionWord}" بنویسید.',
